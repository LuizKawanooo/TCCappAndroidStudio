...
// Query to fetch book details, including the return date
$sql = "SELECT `titulo`, `genero`, `autor`, `editora`, `tombo`, `ano`, `classificacao`, `n_paginas`, `isbn`, `sinopse`, `data_devolucao`, `status_livros` FROM livros WHERE id = ?";
...
if ($result->num_rows > 0) {
    $book = $result->fetch_assoc();
    echo json_encode(array("success" => true, "data" => $book));
} else {
