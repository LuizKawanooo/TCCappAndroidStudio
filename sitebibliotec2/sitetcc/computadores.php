...
// Inicializa as variáveis de mensagem e imagem
$mensagem = '';
$imagemPath = '';
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES['file'])) {
    ...
    // Permite apenas determinados formatos
    ...
    if ($uploadOk == 1) {
        if (move_uploaded_file($_FILES["file"]["tmp_name"], $targetFile)) {
            $mensagem = "O arquivo " . htmlspecialchars(basename($_FILES["file"]["name"])) . " foi enviado com sucesso.";
            $imagemPath = $targetFile; // Armazena o caminho da imagem
        } else {
            $mensagem = "Desculpe, ocorreu um erro ao enviar seu arquivo.";
        }
    }
}

// Verificar a disponibilidade do computador
    ...
    <script>
        function uploadImage() {
            ...
            .then(data => {
                ...
                // Atualiza a imagem se o upload for bem-sucedido
                if (data.includes('O arquivo')) {
                    const imgPath = input.files[0].name; // Modifique para capturar o caminho real
                    document.getElementById('uploadedImage').src = 'uploads/' + imgPath;
                }
            })
...
<form action="" method="POST" enctype="multipart/form-data">
    <label class="custum-file-upload" for="file" onclick="document.getElementById('file').click();">
        <div class="icon">
...
<div id="response">
    ...
    <?php if (!empty($imagemPath)): ?>
        <img id="uploadedImage" src="<?php echo $imagemPath; ?>" alt="Imagem Upload" style="max-width: 100%; margin-top: 20px;">
    <?php endif; ?>
</div>
