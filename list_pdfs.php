...
foreach ($files as $file) {
    if (isset($file['type']) && $file['type'] === 'dir') {
        // Diretório encontrado, busca os PDFs dentro dele
        $category = $file['name'];
        $categoryUrl = $file['_links']['self'] . '/' . $category;
        $categoryResponse = file_get_contents($categoryUrl);
        $categoryFiles = json_decode($categoryResponse, true);
        $pdfs = [];

        foreach ($categoryFiles as $categoryFile) {
            if (isset($categoryFile['name']) && substr($categoryFile['name'], -4) === '.pdf') {
                $pdfs[] = [
                    'name' => $categoryFile['name'],
                    'url' => $categoryFile['download_url']
                ];
            }
        }
